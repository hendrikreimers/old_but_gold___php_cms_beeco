...
/*
   ...
   Zeigt die Menüs an um dort einen Menüpunkt
   löschen zu können.
	...
	if ( $items )
	{
		...
		foreach ( $items as $item )
		{
		    $replacements = array("path"  => $path,
								  ...
								  "url"   => ( $sql->redirect_check($item["id"]) ) ? "javascript:alert('Weiterleitung: Keine Unterpunkte möglich!')" : "?action=delete&id=".$item["id"]."&SID=".$SID);
