...
function initialize($dObj)
{
	...
	$tpl->path .= "attributes/";                                        # Template pfad (für den inhalt)
	...
	if ( $items )
	{
		...
		foreach ( $items as $item )
		{
		    $replacements = array("path"  => $path,
								  ...
								  "url"   => ( $sql->redirect_check($item["id"]) ) ? "javascript:alert('Weiterleitung: Keine Unterpunkte möglich!')" : "?action=attributes&id=".$item["id"]."&SID=".$SID);
